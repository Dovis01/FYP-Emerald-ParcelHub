...
import {
    transferCourierDeliveryRouteAddressToLatAndLng,
...
export const DeliveryRouteMapPlanning = () => {
    ...
    useEffect(() => {
        if (isLoaded) {
            const fetchRouteAddressesLocation = async () => {
                const response = await transferCourierDeliveryRouteAddressToLatAndLng(googleMap.courierDeliveryRouteAddresses);
                if (response.success) {
    ...
    }, [googleMap.courierDeliveryRouteAddresses, isLoaded]);
    ...
    const getRouteOptions = (routeAllAddressesInfo) => {
        const waypointRecords = routeAllAddressesInfo.filter(addressInfo => addressInfo.addressGeoInfo.addressType === 'DST-Parcel Station');
    ...
    const directionsCallback = (routeAllAddressesInfo, res) => {
        if (res !== null) {
            if (res.status === 'OK') {
                googleMap.updateCourierDeliveryOverviewPaths(res.routes[0].overview_path);
            ...
            return (
                <React.Fragment key={`fragment${routeAllAddressesInfo.uniqueId}`}>
                    ...
                    <RenderMarkerTruckIcon key={`truck${routeAllAddressesInfo.uniqueId}`}
                                           ...
                                           currentAddrObj={googleMap.courierCurrentDeliveryAddr}
                    />
                </React.Fragment>
            );
        }
